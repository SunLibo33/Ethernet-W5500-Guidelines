...
void W5500_Init(void)
{
  ...
  Write_W5500_1Byte(0x0018, 0x01);
	...
	Write_W5500_SOCK_1Byte(0,Sn_IMR, 0x04); //Interruption Function ONLY ACK for Receive Information from other Ethernent Temination Device (*^__^*)
...
IntDispose:
	...
	if((i & S0_INT) == S0_INT)//Socket0???? 
	{
		...
		if(j&IR_SEND_OK)//Socket0??????,??????S_tx_process()?????? 
		{
			...
			//When W5500 send information to other Ethernent Temination Device successfully
			//If Sn_IMR (Socket n) [R/W] [0x002C] [Deafault Value = 0xFF] :
			//then W5500 Would send information to other Ethernent Temination Device ForEver!!! Certainly inculding void W5500_Interrupt_Process(void) Function itself!
			
			//If Sn_IMR (Socket n) [R/W] [0x002C] [Set Value = 0x04]      :
			//then W5500 Would Not send following information to other Ethernent Temination Device due to Interruption, only because void W5500_Interrupt_Process(void) Function itself!
			
			//Interruption Function ONLY ACK for Receive Information from other Ethernent Temination Device (*^__^*) source code // Write_W5500_SOCK_1Byte(0,Sn_IMR, 0x04);
