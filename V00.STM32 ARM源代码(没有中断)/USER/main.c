...
void PortEInit()
{
  ...
  GPIOE->MODER |= 0X00000050; //PE2 PE3 Output , PE2 ETH_MOSI and PE3 ETH_Reset
	
	GPIOE->MODER &= 0XFFFFF0FF;
  GPIOE->MODER |= 0X00000000; //PE4 Input , PE4 ETH_MISO
...
{ 
	unsigned int ETH_Receive_Data_Number=0x00;
	unsigned char ii=0;
	unsigned char MyInformation[256];
	
	for(ii=0;ii<=254;ii++)
	  MyInformation[ii]=ii;
	MyInformation[255]=0xff;
